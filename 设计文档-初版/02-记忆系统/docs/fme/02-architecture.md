...
| 组件 | 职责 | 必留理由 |
|---|---|---|
| MandateVerifier | 验证 FAP-1 Mandate 与 FME constraints（purpose、scope、triple） | 安全凭证不能由插件验证 |
...
> - Hippocampus 是 Core 内置的事务/存储抽象，**不**对应任何 Plugin Trait；底层存储通过既有的 ObjectStore / VectorIndex / TagGraph 适配器与内部存储实现协作，但不单独引入 `StoragePlugin`
>
...
```text
写入路径：
  Agent → FAP-1 Invoke → MandateVerifier → Policy → ContentSafety
...
检索路径：
  Agent → FAP-1 Invoke → MandateVerifier → Policy → Scope Filter
...
Core 提供：
  ...
  - 检索流水线骨架（编排，不实现算法）
  - FAP-1 Invoke/Data/Mandate/Receipt 绑定
