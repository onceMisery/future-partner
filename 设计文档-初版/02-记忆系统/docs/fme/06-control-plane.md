...
struct CapabilityTriple {
    capability: MemoryCapability,    // enum，9 值
    layer: MemoryLayer,              // L0 | L1 | L2 | L3 | AUDIT_CHAIN | MEMORY_LAYER_UNSPECIFIED
    action: MemoryAction,            // 13 值（含 UNSPECIFIED）
}
```

完整枚举值见 [purpose-vocabulary.md §3](./purpose-vocabulary.md)；capability_id ↔ triple 映射见 [signing-canonical.md §9.1](./signing-canonical.md)。具体裁决规则详见 [purpose-vocabulary.md](./purpose-vocabulary.md)。

不允许在 FME 中再定义 6 值版 `ActionKind` 或其它简化枚举。
